...
from .._tins import Dot11QoSData, HWAddress, ARP, IP, Dot11Beacon, Utils, RadioTap, Dot11Data
# noinspection PyUnresolvedReferences
...
class RadioTapTest(unittest.TestCase):
    def test_default_constr(self):
        radio = RadioTap()
        assert_true(radio.flags & RadioTap.FrameFlags.FCS)
        eq_(Utils.mhz_to_channel(radio.channel_freq), 1)
        eq_(radio.channel_type, 0xa0)
        eq_(radio.tsft, 0)
        eq_(radio.dbm_signal, -50)
        eq_(radio.antenna, 0)
        eq_(radio.rx_flags, 0)


    def test_constr_buffer(self):
        radio = RadioTap.from_buffer(expected_packet)
        eq_(radio.version, 0)
        eq_(radio.length, 32)
        eq_(radio.rate, 0xc)
        eq_(radio.flags, 0x10)

        assert_true(radio.present & RadioTap.PresentFlags.TSTF)
        assert_true(radio.present & RadioTap.PresentFlags.RATE)
        assert_true(radio.present & RadioTap.PresentFlags.DBM_SIGNAL)
        assert_true(radio.present & RadioTap.PresentFlags.ANTENNA)
        assert_true(radio.present & RadioTap.PresentFlags.CHANNEL_PLUS)

        assert_true(radio.flags & RadioTap.FrameFlags.FCS)
        eq_(radio.channel_type, None)
        eq_(radio.channel_freq, None)
        eq_(radio.tsft, 616089172)
        eq_(radio.dbm_signal, -38)
        eq_(radio.dbm_noise, -96)
        eq_(radio.antenna, 2)

    def test_constr_buf(self):
        radio = RadioTap.from_buffer(expected_packet1)
        eq_(radio.version, 0)
        eq_(radio.length, 26)
        eq_(radio.rate, 2)
        eq_(radio.flags, 0x10)
        assert_true(radio.flags & RadioTap.FrameFlags.FCS)
        eq_(radio.antenna, 1)
        assert_true(radio.rfind_pdu(Dot11Beacon) is not None)

    def test_constr_buf2(self):
        radio = RadioTap.from_buffer(expected_packet2)

        assert_true(radio.present & RadioTap.PresentFlags.RATE)
        assert_true(radio.present & RadioTap.PresentFlags.CHANNEL)
        assert_true(radio.present & RadioTap.PresentFlags.DBM_SIGNAL)
        assert_true(radio.present & RadioTap.PresentFlags.ANTENNA)
        assert_true(radio.present & RadioTap.PresentFlags.RX_FLAGS)

        eq_(radio.version, 0)
        eq_(radio.length, 34)
        eq_(radio.rate, 0x12)
        eq_(radio.flags, 0x02)
        eq_(radio.dbm_signal, -71)
        eq_(radio.channel_type, 192)
        eq_(radio.channel_freq, 2447)
        eq_(radio.antenna, 0)
        assert_true(radio.rfind_pdu(Dot11QoSData) is not None)

    def test_constr_buf3(self):
        radio = RadioTap.from_buffer(expected_packet3)
        ...
        eq_(radio.antenna, 0)
        eq_(radio.dbm_signal, -75)

        assert_true(radio.rfind_pdu(ARP) is not None)

    def test_constr_buf4(self):
        radio = RadioTap.from_buffer(expected_packet4)
        ...
        assert_true(radio.present & RadioTap.PresentFlags.ANTENNA)
        assert_true(radio.present & RadioTap.PresentFlags.RX_FLAGS)
        assert_true(radio.present & RadioTap.PresentFlags.MCS)

        eq_(radio.tsft,         2076020709598 )
        eq_(radio.rx_flags,     0 )
        eq_(radio.antenna,      0 )
        eq_(radio.dbm_signal,   -70 )
        eq_(radio.channel_freq, 2412 )
        eq_(radio.channel_type, 0x0480 )
        eq_(radio.mcs,          (0x27, 0x00, 0x01) )

        assert_true(radio.rfind_pdu(Dot11Data) is not None)

    def test_constr_buf5(self):
        radio = RadioTap.from_buffer(expected_packet5)
        assert_true(radio.present & RadioTap.PresentFlags.DATA_RETRIES)
        assert_true(radio.present & RadioTap.PresentFlags.TX_FLAGS)
        assert_true(radio.present & RadioTap.PresentFlags.MCS)

        eq_(radio.data_retries, 0)
        eq_(radio.tx_flags, 0)
        eq_(radio.mcs, (0x07, 0x00, 0x05))
    ...
    def test_channel(self):
        ...
        eq_(radio.channel_freq, 0xfa23)
        eq_(radio.channel_type, 0xfb6a)

    def test_antenna(self):
        radio = RadioTap()
        radio.antenna = 0x7a
        eq_(radio.antenna, 0x7a)

    def test_padding(self):
        radio = RadioTap()
        radio.padding = 0x7a
        eq_(radio.padding, 0x7a)

    def test_version(self):
        radio = RadioTap()
        radio.version = 0x7a
        eq_(radio.version, 0x7a)

    def test_length(self):
        radio = RadioTap()
        radio.length = 0x7a
        eq_(radio.length, 0x7a)

    def test_dbm_signal(self):
        radio = RadioTap()
        radio.dbm_signal = 0x7a
        eq_(radio.dbm_signal, 0x7a)

    def test_dbm_noise(self):
        radio = RadioTap()
        radio.dbm_noise = 0x7a
        eq_(radio.dbm_noise, 0x7a)

    def test_rx_flags(self):
        radio = RadioTap()
        radio.rx_flags = 0x7afb
        eq_(radio.rx_flags, 0x7afb)
